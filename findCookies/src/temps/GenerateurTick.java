...
public class GenerateurTick extends SujetObservableUneFois implements Runnable {
    ...
    public void run() {
        ...
        while (true) {
            try {
                Thread.sleep(intervalleEntreTicks); // par exemple : on veut un jeu a 60Hz, on permet un check des events toutes les 1/60 de secondes
                super.notifier(); // par exemple : on notifie qu'un tick de 16ms est passé
